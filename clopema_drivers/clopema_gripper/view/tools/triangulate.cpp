...
#define POSITION(x)	(positions[(x)])
#define NORMAL(x)	(orientations[(x)])

#define SQRT3		(1.732050808f)

vector<triangle> triangulate(float positions[][3], float orientations[][3], unsigned int count)
{
	...
	vector<uint16_t>	closests(count, INVALID_ID);
	vector<bool>		covered(count, false);	// a flag to make sure the sensor is covered. If not covered, connect it to its closest neighbor
	// Step 1
	for (uint16_t i = 0; i < count; ++i)
	{
		uint16_t	closest		= INVALID_ID;
		float		min_dist	= 0.0f;
		for (uint16_t j = 0; j < count; ++j)
		{
			...
			if (ZERO(dist))
				continue;
			if (closest == INVALID_ID || dist < min_dist)
			{
				min_dist = dist;
				closest = j;
			}
	...
	{
		if (closests[i] == INVALID_ID) continue;
		float			a[3];
		ASSIGN(a, POSITION(i));
		for (uint16_t j = i+1; j < count-1; ++j)
		{
			if (closests[j] == INVALID_ID) continue;
			float		b[3], ba[3],
					mab[3];
			ASSIGN(b, POSITION(j));
			SUB(ba, a, b);
			...
			ADD(mab, a, b);
			DIV(mab, mab, 2);
			for (uint16_t k = j+1; k < count; ++k)
			{
				if (closests[k] == INVALID_ID) continue;
				float	c[3], cb[3], ac[3],
					mbc[3], mca[3],
					nab[3], nbc[3], nca[3],
					temp[3], temp2[3];
	...
	// Step 3
	typedef pair<uint16_t, uint16_t> PAIR;
	...
	for (uint16_t i = 0; i < count; ++i)
	{
		if (covered[i] || closests[i] == INVALID_ID) continue;
