...
#define IS_RED(x, y) (bmp[(y)*rowBytes+(x)*3+2] == 255 && bmp[(y)*rowBytes+(x)*3+1] == 0 && bmp[(y)*rowBytes+(x)*3] == 0)
	...
	{
		int one = 1;						// Leave place for 1 line of transparent on top of the letters
		int x = accumulatedWidth+i+1;				// Leave place for 1 line of transparent on the start, end and between each letter
		fwrite(&x, 4, 1, f);
		fwrite(&one, 4, 1, f);
		...
		accumulatedWidth += lettersInBMP[i].width;
	}
	...
	accumulatedWidth += 257;					// 255 transparent lines between letters and 2 on the start and the end
	int textureHeight = 1, textureWidth = 1;
	while (textureHeight < maxHeight+2) textureHeight <<= 1;	// one row on top and one on the bottom for transparent lines
	while (textureWidth < accumulatedWidth) textureWidth <<= 1;
	...
	for (int y = 0; y < maxHeight; ++y)				// Write the font image row by row
	{
		...
		fputc(0, f);
		++currentWidth;
		for (unsigned int i = 0; i < lettersInBMP.size() && i < 256; ++i)
		{
	...
	for (int y = maxHeight; y < textureHeight+1; ++y)
		for (int x = 0; x < textureWidth; ++x)
			fputc(0, f);	// padding
	delete[] bmp;
	fclose(f);
...
int shFontConvertToBMP(const char *fontFile, const char *bmpFile)
{
	...
	for (int y = 0; y < imageHeight; ++y)
	{
		int currentLetter = 0;
		fgetc(f);					// should be 0
		bmpImage[(imageHeight-1-y)*rowBytes+2] = 255;
		bmpImage[(imageHeight-1-y)*rowBytes+1] = 0;
		bmpImage[(imageHeight-1-y)*rowBytes] = 0;
		for (int x = 1; x < imageWidth; ++x)
		{
			while (x >= lettersInFont[currentLetter].x+lettersInFont[currentLetter].width)
			{
				if (currentLetter <= 255)	// if already 256, then I'm seeing paddings and not moving to next letter
				{
					fgetc(f);		// another 0
					bmpImage[(imageHeight-1-y)*rowBytes+x*3+2] = 255;
					bmpImage[(imageHeight-1-y)*rowBytes+x*3+1] = 0;
					bmpImage[(imageHeight-1-y)*rowBytes+x*3] = 0;
					++currentLetter;
					++x;
				}
				else
			...
			unsigned char c = 255-(unsigned char)fgetc(f);
			if (feof(f))
			{
				LOG("Font file \"%s\" is incomplete", fontFile);
				delete[] bmpImage;
			...
			if (currentLetter > 255 || lettersInFont[currentLetter].y > y || lettersInFont[currentLetter].y+lettersInFont[currentLetter].height <= y)
			{
				bmpImage[(imageHeight-1-y)*rowBytes+x*3+2] = 255;
				bmpImage[(imageHeight-1-y)*rowBytes+x*3+1] = 0;
				bmpImage[(imageHeight-1-y)*rowBytes+x*3] = 0;
			...
			{
				bmpImage[(imageHeight-1-y)*rowBytes+x*3+2] = c;
				bmpImage[(imageHeight-1-y)*rowBytes+x*3+1] = c;
				bmpImage[(imageHeight-1-y)*rowBytes+x*3] = c;
			}
		}
		for (int x = imageWidth*3; x < rowBytes; x += 3)
		{
			bmpImage[(imageHeight-1-y)*rowBytes+x+2] = 255;
			bmpImage[(imageHeight-1-y)*rowBytes+x+1] = 0;
			bmpImage[(imageHeight-1-y)*rowBytes+x] = 0;
		}
		for (int x = rowBytes-rowBytes%3; x < rowBytes; ++x)
			bmpImage[(imageHeight-1-y)*rowBytes+x] = 0;
	}
...
int shFontLoad(const char *fontFile)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 4, imageWidth, imageHeight, 0, GL_BGRA, GL_UNSIGNED_BYTE, texture);
	delete[] texture;
	for (int i = 0; i < 256; ++i)
	{
